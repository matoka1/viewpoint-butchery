...
function addToCartPOS() {
    ...
    const amount = parseFloat(document.getElementById('posAmount')?.value) || 0;
    if (!qty && !amount) {
        showToast('Enter quantity or amount', 'warning');
        return;
    }
...
async function viewOrderDetails(orderId) {
    const { data: order } = await supabaseClient.from('orders').select('*, order_items(*, products(*))').eq('id',
        orderId).single();
    if (!order) return;
...
async function loadProductDropdown() {
    ...
    select.innerHTML = productsData.map(p =>
        `<option value="${p.id}">${getEmoji(p.name)} ${p.name}</option>`).join('');
}
...
async function loadUsers() {
    ...
    let html =
        '<div class="table-wrapper"><table class="data-table"><thead><tr><th>👤 Name</th><th>📧 Email</th><th>👑 Role</th><th>📊 Status</th><th>⚙️ Actions</th></tr></thead><tbody>';
    usersData.forEach(u => {
